...
impl MatchOutput {
    fn no_output(tokens: TokenStream2) -> Self {
        Self { tokens, var_count: 0 }
    }

    fn with_output(tokens: TokenStream2, var_count: usize) -> Self {
...
impl Matcher for ElementMatcher {
    fn perform_match(&self, formula: TokenStream2) -> MatchOutput {
        let successfull_match = match self {
            ElementMatcher::Any => {
                return MatchOutput::no_output(quote! { Some(()) });
            },
            ElementMatcher::X => {
                return MatchOutput::with_output(quote! { Some(#formula) }, 1);
            },
            ElementMatcher::Expression(expr) => {
                return MatchOutput::no_output(quote! { (#formula == #expr).then_some(()) });
            },
            ElementMatcher::WithoutInner(element) => {
                let element_string = element.as_pattern();
                return MatchOutput {
                    tokens: quote! {matches!(#formula, #element_string {..}.then_some(()))},
                    var_count: 0,
                };
            },
