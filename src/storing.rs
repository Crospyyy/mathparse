...
impl FormulaStore {
    pub(crate) fn new_empty() -> Self {
        FormulaStore { signatures: Signatures::new_empty(), formulas: HashMap::new(), parameter_mappings: HashMap::new() }
    }
...
impl Element {
    fn insert_symbol(&mut self, insert:&InsertionElement) -> Result<(), String> {
        match self {
            Element::Brackets(elements) |Element::Plus(elements)|Element::Multiply(elements)|Element::Function { arguments:elements,.. }=> for e in elements{
            ...
            Element::Number(_)|Element::Variable(_)|Element::VariableOrFunction(_) | Element::String(_) => {}
        }
        if self.get_name().is_some_and(|n|n==insert.name) {
            let types_equal = if insert.arguments.is_some() { matches!(self, Element::Function {..}) } else { matches!(self,Element::Variable(_)) };
            if !types_equal { return Err("Insertion element and formula don't match".to_owned()) }
            if let Some(insert_args)=&insert.arguments {
                ...
                for (in_arg,val) in insert_args.iter().zip(self_arguments) {
                    new_formula.insert_symbol(&InsertionElement{name:in_arg.clone(),arguments:None, formula:val.clone()})?
                }
                *self = new_formula;
            } else {
...
fn test_insert_symbols(){
    ...
    formula.print_debug();
    formula.insert_symbol(&InsertionElement{name:"f".to_owned(),arguments:Some(vec!["x".to_owned(),"y".to_owned()]),formula:fun}).unwrap();
    formula.print_debug();
}